...
.DESCRIPTION
   Pause a running scan on a Nessus server.
.EXAMPLE
    Suspend-AcasScan -SessionId 0 -ScanId 46
    ...
    PS C:\> Get-AcasScan -SessionId 0 -Status Paused
...
.DESCRIPTION
   Resume a paused scan on a Nessus server.
.EXAMPLE
   Resume-AcasScan -SessionId 0 -ScanId 46
    ...
    PS C:\> Get-AcasScan -SessionId 0 -Status Running
...
.DESCRIPTION
   Cancel a scan on a Nessus server.
.EXAMPLE
   Stop-AcasScan -SessionId 0 -ScanId 46
...
.DESCRIPTION
   Launch a scan on a Nessus server.
.EXAMPLE
   Start-AcasScan -SessionId 0 -ScanId 15 -AlternateTarget 192.168.11.11,192.168.11.12
...
.DESCRIPTION
   Get scans present on a Nessus server.
.EXAMPLE
    Get-AcasScan -SessionId 0 -Status Completed
...
.DESCRIPTION
   Long description
.EXAMPLE
   Show-AcasScanHostDetail -SessionId 0 -ScanId 46 -HostId 31 | fl
...
.EXAMPLE
   Show-AcasScanHost -SessionId 0 -ScanId 46
...
.DESCRIPTION
   Shows the history of times ran for a specific scan in a Nessus server.
.EXAMPLE
   Show-AcasScanHistory -SessionId 0 -ScanId 46
...
.DESCRIPTION
   Deletes a scan result from a Nessus server.
.EXAMPLE
    Get-AcasScan -SessionId 0 -Status Imported | Remove-AcasScan -SessionId 0 -Verbose
...
.DESCRIPTION
   Deletes a history result from a Nessus server scan.
.EXAMPLE
    Remove-AcasScanHistory -SessionId 0 -ScanId 263 -HistoryId 1519 -Verbose
...
.DESCRIPTION
   Get all scan templates available on a Nessus server.
.EXAMPLE
   Get-AcasScanTemplate -SessionId 0
#>
