function Resolve-NetworkName {
    ...
    # resolve IP address
    try {
        Write-PSFMessage -Level Verbose -Message "Resolving $cName using .NET.Dns GetHostEntry"
    ...
    try {
        Write-PSFMessage -Level Verbose -Message "Resolving $ipaddress using .NET.Dns GetHostByAddress"
        $fqdn = [System.Net.Dns]::GetHostByAddress($ipaddress).HostName
    } catch {
        Write-PSFMessage -Level Verbose -Message "Failed to resolve $ipaddress using .NET.Dns GetHostByAddress"
